...
const budgetThemeRuntime = `<script id="chod-theme-runtime">(()=>{const root=document.documentElement;const key="chod-theme-mode:v1";const normalize=(value)=>value==="light"?"light":"dark";const apply=(value)=>{const mode=normalize(value);root.dataset.theme=mode;root.style.colorScheme=mode;};const shellRoot=()=>{try{if(window.parent!==window)return window.parent.document.documentElement;if(window.opener&&!window.opener.closed)return window.opener.document.documentElement;}catch{}return null;};const initialRoot=shellRoot();const queryTheme=new URL(window.location.href).searchParams.get("theme");let stored="";try{stored=window.localStorage.getItem(key)||"";}catch{}apply(initialRoot?.dataset.theme||queryTheme||stored||"dark");if(initialRoot){new MutationObserver(()=>apply(initialRoot.dataset.theme)).observe(initialRoot,{attributes:true,attributeFilter:["data-theme"]});}window.addEventListener("storage",(event)=>{if(event.key===key)apply(event.newValue);});window.addEventListener("message",(event)=>{if(event.origin!==window.location.origin||event.data?.type!=="chod:theme")return;apply(event.data.mode);});})();</script>`;
